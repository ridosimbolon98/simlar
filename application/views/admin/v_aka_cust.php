...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        <div class="mb-2">
          <button class="btn btn-sm btn-success" data-toggle="modal" data-target="#add-akanew"><i class="fa fa-plus-square text-light"></i> Input AKA Baru</button>
        </div>
                    ...
                    <tr class="text-center">
                      ...
                      <td class="text-center btn-aka">
                        <a id="input_aka" class="btn btn-sm btn-primary" 
                          ...
                          data-jlh_pakai="<?= $row->jlh_pakai; ?>">
                          <i class="fa fa-pen-square text-light"></i> Input AKA
                        </a>
                        <button id="print_struk" data-idaka="<?= $row->id_aka ?>" class="btn btn-sm btn-secondary"><i class="text-light fa fa-print"></i> Cetak</button>
                      </td>
                    </tr>
                    <?php endforeach; ?>
                </table>
        ...
        <div class="modal-body">
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              ...
              <input type="text" hidden name="tahun2" id="tahun2">
              <input type="text" hidden name="bulan" id="bulan1">
              <input type="month" readonly class="form-control"  id="periode_aka">
            </div>
          </div>
          <div class="form-group row">
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <input id="aka_akhr1" type="number" name="aka_akhir" step="1" class="form-control" id="aka_akhir">
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="submit" class="btn btn-primary" onclick="return confirm('Apakah anda yakin submit AKA baru ini?')"><i class="fa fa-save text-light"></i> Submit</button>
...
<!-- Modal Add Aka new -->
<div class="modal fade bd-example-modal-lg" id="add-akanew" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Input AKA Pelanggan Baru</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="<?= base_url('admin/input_aka') ?>" method="post">
        <div class="modal-body">
          <div class="form-group row">
            <label for="nama_cust" class="col-sm-2 col-form-label">Nama</label>
            <div class="col-sm-10">
              <select name="cust" class="form-control" required>
                <option value="" hidden disabled-selected>--Pilih Data Pelanggan--</option>
                <?php foreach($pelanggan as $row): ?>
                  <option value="<?= $row->cid ?>"><?= $row->nama ?></option>
                <?php endforeach; ?>
              </select>
            </div>
          </div>
          <div class="form-group row">
            <label for="alamat_cust" class="col-sm-2 col-form-label">Periode</label>
            <div class="col-sm-10">
              <input type="month" class="form-control" value="<?= date('Y-m') ?>" name="periode" required>
            </div>
          </div>
          <div class="form-group row">
            <label for="periode" class="col-sm-2 col-form-label">AKA Akhir</label>
...
$(document).on("click", "#input_aka", function () {
  ...
  $.ajax({
    ...
    success: function(msg){
      ...
      var idx = parseInt(kmp_data.length) - 1;
      if ((parseInt(kmp_data[idx].bulan) + 1) <= 12) {
        var nm = (parseInt(kmp_data[idx].bulan) + 1);
        if (nm < 10) {
          var next_month = '0'+nm;
        } else {
          var next_month = nm;
...
<script>
  ...
  let aka_akhr = document.getElementById("aka_akhr");
  let aka_akhr1 = document.getElementById("aka_akhr1");

  aka_akhr.addEventListener("input", () => {
    $("#stnd_mtr").val(aka_akhr.value);
  });
  
  aka_akhr1.addEventListener("input", () => {
    $("#stnd_mtr1").val(aka_akhr1.value);
  });
</script>
